...
export class World {
    ...
    setViewChange()
    {
        const basePosition = this.terrain.board.getWorldPosition(new THREE.Vector3())
    ...
    setTerrain() {
        ...
        const bodyDesc = this.game.RAPIER.RigidBodyDesc.fixed().setTranslation( 10 + this.offset.x, 1.2, this.offset.z )

        this.assessBody = this.game.physics.world.createRigidBody(bodyDesc)
        const mainCollider = this.game.RAPIER.ColliderDesc.cuboid(5.5, 2, 5.5)
        this.game.physics.world.createCollider(mainCollider, this.assessBody);
    }
